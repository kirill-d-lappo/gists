...
map('', '<C-_>', [[ <CMD>lua require("Comment.api").toggle_current_blockwise()<CR><down>]], {noremap = true, silent = false })
...
map('n', '<F9>', ':so ~/.config/nvim/init.lua<CR>:so ~/.config/nvim/lua/plugins.lua<CR>:so ~/.config/nvim/lua/settings.lua<CR>:so ~/.config/nvim/lua/keymaps.lua<CR>', { noremap = true })
